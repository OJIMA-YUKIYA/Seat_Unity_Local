...
public class LowerLimbMotionController_WalkDemo : LowerLimb6MotorBase
{
    [SerializeField] WalkDemoMainController mainController;
    [SerializeField] UDPReceiver udpReceiver;


    public string host = "192.168.11.25";
    ...
    [SerializeField, Range(-55, 105)] public float leftPedalUp1 = 25; //左ペダル下端上昇目標値[mm]
    [SerializeField, Range(-55, 105)] private float leftPedalDown1 = 0; //左ペダル下端下降目標値[mm]
    [SerializeField, Range(-90, 90)] float leftSliderForward1 = 36; ////左スライダ前進目標値[mm]
    [SerializeField, Range(-90, 90)] float leftSliderBackward1 = -24; //左スライダ後退目標値[mm]


    [SerializeField, Range(-55, 105)] public float rightPedalUp1 = 25; //右ペダル下端上昇目標値[mm]
    [SerializeField, Range(-55, 105)] private float rightPedalDown1 = 0; //右ペダル下端下降目標値[mm]

    [SerializeField, Range(-90, 90)] float rightSliderForward1 = 36; //右スライダ後退目標値[mm]
    [SerializeField, Range(-90, 90)] float rightSliderBackward1 = -24; //右スライダ後退目標値[mm]

    //Yaw回転
    [SerializeField, Range(-18, 18)] float leftRotationAngle1 = -1.5f; //左足前進時Yaw回転角度[degree]
    [SerializeField, Range(-18, 18)] float leftRotationAngle2 = 1.5f; //左足後退時Yaw回転角度[degree]

    [SerializeField, Range(-18, 18)] float rightRotationAngle1 = -1.5f; //右足前進時Yaw回転角度[degree]
    [SerializeField, Range(-18, 18)] float rightRotationAngle2 = 1.5f; //右足後退時Yaw回転角度[degree]
    ...
    void OnApplicationQuit() //Scene閉じたとき
    {
        if (walk)
        {
            sendText = "stop" + "," + "/";
