...
public class OnlySeatController : MonoBehaviour
{
    [SerializeField]
    ...
    /// アクチュエーターの長さ設定(mm)
    /// </summary>
    [SerializeField, HeaderAttribute("Length"), Range(-50, 50)]//スライダの範囲を設定
    ...
    void Start ()
	{
        ...
        //StartCoroutine("WalkToLeft");
        //StartCoroutine("WalkToRight");
    ...
    void Update ()
	{
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow))
        {
            Left = true;
            Right = false;
            // walkCycle = 1.4f - Time.deltaTime;
            WalkLeft();
            command = true;//mainController返信用
            walkCycle = 0;
            //while (mainController.walkStraight || mainController.walkRight || mainController.walkStop || mainController.walkBack)//yaw回転量更新
            //{
            //    // if(time >= cycletime){}
        ...
        if (Input.GetKeyDown(KeyCode.RightArrow))
        {
            ...
            //    yield return new WaitForSeconds(1.4f);
            //while (mainController.walkStraight || mainController.walkLeft || mainController.walkStop || mainController.walkBack)//yaw回転量更新
            //{
            //     if(time >= cycletime){}
            //    yield return new WaitForSeconds(1.4f);
            //    WalkLeft();
        ...
        //if (mainController.walkBack)//歩行停止
        ...
        //if (Input.GetKeyDown(KeyCode.S))//パラメータ変更
        //{
    ...
    void SetParameterToLength()
    {
        ...
        F_4 = Math.Pow(Math.Pow((a - 1) * 160, 2.0) + Math.Pow((d * 160), 2.0) + Math.Pow(g * 160 + z_c, 2.0), 0.5) - l0;
        F_5 = Math.Pow(Math.Pow((-a + b + 1) * 160, 2.0) + Math.Pow(((-d + e - 1) * 160), 2.0) + Math.Pow((-g + h) * 160 + z_c, 2.0), 0.5) - l0;
        F_6 = Math.Pow(Math.Pow((-a - b + 1) * 160, 2.0) + Math.Pow(((-d - e + 1) * 160), 2.0) + Math.Pow((-g - h) * 160 + z_c, 2.0), 0.5) - l0;
