...
public class UDPCom2:MonoBehaviour
{
    ...
    //出力パルス（送信）
    public int[] targetPulseUp = new int[6] { 0, 0, 0, 0,0,0 };//昇降／前進時の目標パルス（左ペダル、左スライダ、右ペダル、右スライダ）[pulse]
    public int[] targetPulseDown = new int[6] { 0, 0, 0, 0, 0, 0 };//下降／後退時の目標パルス（左ペダル、左スライダ、右ペダル、右スライダ）[pulse]
    public int[] driveTimeUp = new int[6] { 560, 560, 560, 560, 560, 560 };//昇降／前進時の駆動時間（左ペダル、左スライダ、右ペダル、右スライダ）[pulse]
    public int[] driveTimeDown = new int[6] { 280, 840, 280, 840, 840, 840 };//下降／後退時の駆動時間（左ペダル、左スライダ、右ペダル、右スライダ）[pulse]
    ...
    float leftRotationAngleForward = 9f;//左足前進時Yaw回転角度[degree]
    [SerializeField, Range(-18, 18)]
    float leftRotationAngleBackward = 0f;//左足後退時Yaw回転角度[degree]
    [SerializeField, Range(-18, 18)]
    float rightRotationAngleForward = 9f;//右足前進時Yaw回転角度[degree]
    [SerializeField, Range(-18, 18)]
    float rightRotationAngleBackward = 0f;//右足後退時Yaw回転角度[degree]
	...
	void Update ()
	{
        if (Input.GetKeyDown(KeyCode.Space))
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                sendText += targetPulseUp[i].ToString() + "," + targetPulseDown[i].ToString() + "," + driveTimeUp[i].ToString() + "," + driveTimeDown[i].ToString() + ",";
            }
            sendText += "e";//終わりの目印
    ...
    void targetCalculate()
    {
        ...
        targetPulseUp[4] = (int)(leftRotationAngleForward * 10000*11 / 120);//回転角度*（駆動モータ1回転のパルス量/モータ1回転でのレール上回転角度）
        targetPulseDown[4] = (int)(leftRotationAngleBackward * 10000*11/120);//回転角度*（駆動モータ1回転のパルス量/モータ1回転でのレール上回転角度）
        targetPulseUp[5] = (int)(rightRotationAngleForward * 10000 *11/ 120);//回転角度*（駆動モータ1回転のパルス量/モータ1回転でのレール上回転角度）
        targetPulseDown[5] = (int)(rightRotationAngleBackward *10000 *11/ 120);//回転角度*（駆動モータ1回転のパルス量/モータ1回転でのレール上回転角度）
    }
}
